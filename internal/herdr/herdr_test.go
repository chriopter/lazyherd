...
func TestOpenCockpitFocusesExistingOrOpens(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"focus", `{"pane_id":"w1:p9","workspace_id":"w1","label":"lazyherd"}`, "plugin pane focus w1:p9\n"},
		{"other workspace", `{"pane_id":"w2:p9","workspace_id":"w2","label":"lazyherd"}`, "plugin pane open --plugin x.y --entrypoint cockpit --workspace w1 --focus\n"},
		{"other label", `{"pane_id":"w1:p9","workspace_id":"w1","label":"shell"}`, "plugin pane open --plugin x.y --entrypoint cockpit --workspace w1 --focus\n"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			scriptOnPath(t, "herdr", `case "$1 $2" in "pane list") printf '%s' '{"result":{"panes":[`+tt.panes+`]}}';; *) echo "$*" >> "$HERDR_CALLS";; esac`)
			os.Remove(log)
